...
function agregarSelectorcategoria(){
  ...
  let estados = ['Administración / Oficina','Tecnología de la Información', 'Finanzas / Contabilidad', 'Marketing', 'Ventas', 'Recursos Humanos', 'Operaciones', 'Ingeniería'] ; 
...
agregarMensajeChatbot("Mantente al día con tus postulaciones. Por favor ingresa tu correo electrónico");
...
function manejarFlujoSeguimiento(userInput) {
      if (estadoConversacion === 'preguntaCorreo') {
        if (validateEmail(userInput)) {
            setTimeout(function() {
                agregarMensajeChatbot(`Buscando estatus de tus postulaciones asociadas al correo: ${userInput}`);
            }, 1000);
...
function confirmacionAyuda() {
    ...
    setTimeout(() => {
        let contenedorMensajes = document.querySelector('.chatbot-body');
        let botones = contenedorMensajes.querySelectorAll('.chatbot-message-buttons');
