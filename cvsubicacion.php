<?php
$UbicacionSeleccionada = $_POST['seleccion-cate'];
...
if (($handle = fopen($nombreArchivoCSV, "r")) !== FALSE) {
    ...
    $indiceNombre = array_search('Nombre de la posición', $encabezados);
    $indiceEstado = array_search('Ubicación', $encabezados);
    $indiceEP = array_search('Estado de publicación', $encabezados); // EP = ESTADO DE RIQUISICIÓN
    $indiceLink = array_search('ID de requisición de personal', $encabezados); 
    ...
    while (($fila = fgetcsv($handle, 1000, ",")) !== FALSE) {
        if ($fila[$indiceEstado] === $UbicacionSeleccionada && $fila[$indiceEP] === "Publicado") {
